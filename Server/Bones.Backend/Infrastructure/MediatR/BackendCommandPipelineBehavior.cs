    ...
    where TRequest : notnull
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        finally
        {
            ...
            {
                Log.Error(
                    $"Command Errored [{requestId}] | {exception?.GetType().FullName ?? "No Name"} | {stopwatch.ElapsedMilliseconds}ms | {exception?.Message ?? "No Message"} | {typeof(TRequest).FullName} | {request.ToString()}");
            }
        }
    }
}
...
{
    public async Task<QueryResponse<TResponse>> Handle(TRequest request, RequestHandlerDelegate<QueryResponse<TResponse>> next, CancellationToken cancellationToken)
    {
        ...
        finally
        {
            ...
            {
                Log.Error($"Query Errored [{requestId}] | {exception?.GetType().FullName ?? "No Name"} | {stopwatch.ElapsedMilliseconds}ms | {exception?.Message ?? "No Message"} | {typeof(TRequest).FullName} | {request.ToString()}");
            }
        }
    }
}
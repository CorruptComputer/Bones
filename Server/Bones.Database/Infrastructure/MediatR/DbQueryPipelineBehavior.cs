    ...
    where TRequest : notnull
{
        ...
        RequestHandlerDelegate<DbQueryResponse<TResponse>> next, CancellationToken cancellationToken)
    {
        ...
        finally
        {
            ...
            {
                Log.Error(
                    $"DB Query Errored [{requestId}] | {exception?.GetType().FullName ?? "No Name"} | {stopwatch.ElapsedMilliseconds}ms | {exception?.Message ?? "No Message"} | {typeof(TRequest).FullName} | {request.ToString()}");
            }
        }
    }
}
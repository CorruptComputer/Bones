...
/// </summary>
[JsonSerializable(typeof(CreateAssetVersionAction))]
public sealed record class CreateAssetVersionAction : AssetActionBase
{
    /// <summary>
    ...
    internal override async Task<IRequest<CommandResponse>> ToInternalAsync(BonesUser user, ISender sender)
    {
        ...
        if (layout?.LatestVersion is null)
        {
            ...
            {
                RequestModel = nameof(CreateAssetVersionAction),
        ...
        return new CreateAssetVersion.Command(AssetId, layout.Id, Title, fieldValues, ActionDateTime, user);
    }
    ...
    internal override Task<AssetActionResponse> FromInternalAsync(CommandResponse result, BonesUser user, ISender sender)
    {
        if (!result.Success)
        {
            throw new BonesException("CreateAssetVersionAction.FromInternalAsync called with a failed CommandResponse");
        }

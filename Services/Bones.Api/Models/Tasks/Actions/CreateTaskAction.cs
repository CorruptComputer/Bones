...
/// </summary>
[JsonSerializable(typeof(CreateTaskAction))]
public sealed record class CreateTaskAction : TaskActionBase
{
    /// <summary>
    ...
    /// <summary>
    ///   The ID of the  queue to create the  in
    /// </summary>
    ...
    internal override async Task<IRequest<CommandResponse>> ToInternalAsync(BonesUser user, ISender sender)
    {
        ...
        return new CreateTaskInQueue.Command(TaskQueueId, layout.Id, layout.Current.Id, Title, fieldValues, ActionDateTime, user);
    }

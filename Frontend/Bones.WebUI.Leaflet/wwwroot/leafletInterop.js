...
window.leafletInterop = {
    create: function (map, objectReference) {
        let leafletMap = L.map(map.id, {
            ...
            maxBounds: map.maxBounds?.item1 && map.maxBounds.item2 ? L.latLngBounds(map.maxBounds.item1, map.maxBounds.item2) : undefined,
        });
    ...
    addTilelayer: function (mapId, tileLayer, objectReference) {
        const layer = L.tileLayer(tileLayer.urlTemplate, {
            ...
            bounds: tileLayer.bounds?.item1 && tileLayer.bounds.item2 ? L.latLngBounds(tileLayer.bounds.item1, tileLayer.bounds.item2) : undefined,
            // ---
    ...
    addPolyline: function (mapId, polyline, objectReference) {
        const layer = L.polyline(shapeToLatLonArray(polyline.shape), createPolyline(polyline));
    ...
    updatePolyline: function (mapId, polyline) {
        ...
        if (layer !== undefined) {
            layer.setLatLngs(shapeToLatLonArray(polyline.shape));
        }
    },
    addPolygon: function (mapId, polygon, objectReference) {
        const layer = L.polygon(shapeToLatLonArray(polygon.shape), createPolyline(polygon));
    ...
    updatePolygon: function (mapId, polygon) {
        ...
        if (layer !== undefined) {
            layer.setLatLngs(shapeToLatLonArray(polygon.shape));
        }
    },
...
function shapeToLatLonArray(shape) {
    let latlons = [];
    shape.forEach(pts => {
        let ll = [];
...
function mapEvents(mapElement, objectReference, eventHandlerDict) {
    for (let key in eventHandlerDict) {
        ...
        mapElement.on(key, function (eventArgs) {
            if (eventArgs.latlng) {
                // We use LatLon instead of LatLng, just translate it from Leaflet here
                eventArgs.LatLon = {
                    lat: eventArgs.latlng.lat,
                    lon: eventArgs.latlng.lng
                };
            }

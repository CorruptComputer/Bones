...
public class CreateItemLayoutVersionDb(BonesDbContext dbContext) : IRequestHandler<CreateItemLayoutVersionDb.Command, CommandResponse>
{
    ...
    public sealed record Command(Guid ItemLayoutId, string Name, ItemLayoutUse LayoutUse, Dictionary<int, Guid> FieldVersions, Dictionary<int, (string name, AssignmentType assType, SelectionType selType, List<string> states)> AssigneeSlots) : IRequest<CommandResponse>;
    ...
    public class Validator : AbstractValidator<Command>
    {
        ...
        public Validator()
        {
            ...
            RuleFor(x => x.AssigneeSlots).NotEmpty()
                ...
                .Must(x =>
                {
                    IEnumerable<IGrouping<string, KeyValuePair<int, (string name, AssignmentType assType, SelectionType selType, List<string> states)>>> g = x.GroupBy(f => f.Value.name);
                    if (!g.All(f => f.Count() == 1))
                    {
                        return false;
                    }
                ...
                .Must(x =>
                {
                    IEnumerable<IGrouping<int, KeyValuePair<int, (string name, AssignmentType assType, SelectionType selType, List<string> states)>>> g = x.GroupBy(f => f.Key);
                    if (!g.All(f => f.Count() == 1))
                    {
                        return false;
                    }

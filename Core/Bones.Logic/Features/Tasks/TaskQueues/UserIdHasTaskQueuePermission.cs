...
/// <inheritdoc />
public class UserIdHasTaskQueuePermission(UserManager<BonesUser> userManager, RoleManager<BonesRole> roleManager, ISender sender)
    : IRequestHandler<UserIdHasTaskQueuePermission.Query, QueryResponse<bool>>
{
    /// <summary>
    ...
    public sealed record Query(Guid TaskQueueId, Guid UserId, string Claim) : IRequest<QueryResponse<bool>>;
    ...
    public async Task<QueryResponse<bool>> Handle(Query request, CancellationToken cancellationToken)
    {
        ...
        bool? initiativePermission = await sender.Send(
            new UserIdHasInitiativePermission.Query(taskQueue.Initiative.Id, request.UserId, request.Claim),
            cancellationToken);

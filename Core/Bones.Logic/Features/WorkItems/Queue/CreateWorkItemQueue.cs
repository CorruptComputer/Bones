...
public sealed class CreateWorkItemQueue(ISender sender) : IRequestHandler<CreateWorkItemQueue.Command, CommandResponse>
{
    ...
    public async Task<CommandResponse> Handle(Command request, CancellationToken cancellationToken)
    {
        ...
        {
            return await sender.Send(new CreateWorkItemQueueDb.Command(request.Name, request.InitiativeId), cancellationToken);
        }
        ...
        const string perm = BonesClaimTypes.Role.WorkItemQueue.CREATE_QUEUE;

...
public record ChangeAccountEmailDbCommand(Guid AccountId, string Email) : IRequest<CommandResponse>;

internal class ChangeAccountEmailDbHandler(BonesDbContext dbContext, ISender sender)
    : IRequestHandler<ChangeAccountEmailDbCommand, CommandResponse>
{
    public async Task<CommandResponse> Handle(ChangeAccountEmailDbCommand request,

...
public class CreateItemDb(BonesDbContext dbContext) : IRequestHandler<CreateItemDb.Command, CommandResponse>
{
    /// <summary>
    ...
    {
        Queue? queue = await dbContext.Queues.FirstOrDefaultAsync(i => i.Id == request.QueueId, cancellationToken);
        if (queue == null)
        {
            return new()
            {
        ...
        LayoutVersion? layoutVersion = await dbContext.LayoutVersions.Include(layoutVersion => layoutVersion.Fields).FirstOrDefaultAsync(i => i.Id == request.LayoutVersionId, cancellationToken);
        ...
        {
            ItemField? field = layoutVersion.Fields.FirstOrDefault(f => f.Name == key);
            if (field == null)
            {
                return new()
                {

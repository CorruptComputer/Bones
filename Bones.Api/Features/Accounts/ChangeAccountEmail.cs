...
public record ChangeAccountEmailCommand(Guid AccountId, string Email) : IRequest<CommandResponse>;

internal class ChangeAccountEmailHandler(ISender sender)
    : IRequestHandler<ChangeAccountEmailCommand, CommandResponse>
{
    public async Task<CommandResponse> Handle(ChangeAccountEmailCommand request,
